...
public class BST {
    ...
    public int countSmaller(int num) { return countSmaller(root, num); }
    private int countSmaller(Node curr, int num)
    {
        if (curr == null) return 0;
        System.out.println("at "+ curr.key);
        if (num < curr.key)
        {
            int x = countSmaller(curr.left, num);
            return x;
        }
        else if (num >= curr.key)
        {
            int x = countSmaller(curr.left, num);
            int y = countSmaller(curr.right, num);
            return x+y+1;
        }
        else
        {
